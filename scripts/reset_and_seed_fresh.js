...
async function resetAndSeedFresh() {
  ...
  // Use TRUNCATE CASCADE to cleanly wipe all tables in a single transaction
  const tables = [
    'ExpenseClaim',
    'OnboardingDocument',
    'OnboardingCandidate',
  ...
  console.log('✔ Provisioned 3 Branch Locations (Pondicherry HQ, Chennai, Bangalore)\n');
  ...
  const pl = await prisma.leaveType.create({
    data: { organizationId: org.id, name: 'Paid Annual Leave', code: 'PL', daysAllowed: 18, isPaid: true }
  });
  const sl = await prisma.leaveType.create({
    data: { organizationId: org.id, name: 'Sick Leave', code: 'SL', daysAllowed: 12, isPaid: true }
  });
  const cl = await prisma.leaveType.create({
    data: { organizationId: org.id, name: 'Casual Leave', code: 'CL', daysAllowed: 12, isPaid: true }
  });
  ...
  const holidays = [
    ...
    { name: 'Pongal / Makar Sankranti', date: new Date('2026-01-15'), type: 'GOVERNMENT' },
    { name: 'Republic Day', date: new Date('2026-01-26'), type: 'GOVERNMENT' },
    { name: 'May Day (Labor Day)', date: new Date('2026-05-01'), type: 'COMPANY' },
    { name: 'Independence Day', date: new Date('2026-08-15'), type: 'GOVERNMENT' },
    { name: 'Gandhi Jayanti', date: new Date('2026-10-02'), type: 'GOVERNMENT' },
    { name: 'Diwali Festival', date: new Date('2026-11-08'), type: 'GOVERNMENT' },
  ...
  for (const u of usersToCreate) {
    ...
    await prisma.leaveBalance.createMany({
      data: [
        { organizationId: org.id, employeeId: emp.id, leaveTypeId: pl.id, year: 2026, allocatedDays: 18, usedDays: 2 },
        { organizationId: org.id, employeeId: emp.id, leaveTypeId: sl.id, year: 2026, allocatedDays: 12, usedDays: 1 },
        { organizationId: org.id, employeeId: emp.id, leaveTypeId: cl.id, year: 2026, allocatedDays: 12, usedDays: 0 }
      ]
    });

    console.log(`✔ Created User & Employee: ${u.email} (${u.role}) — ${u.firstName} ${u.lastName} [${u.employeeCode}]`);
  }
  ...
  await prisma.expenseClaim.create({
    data: {
      ...
      description: 'Travel from Chennai International Airport to OMR Tech Park for client meetings.',
      status: 'PENDING'
    }
  });
