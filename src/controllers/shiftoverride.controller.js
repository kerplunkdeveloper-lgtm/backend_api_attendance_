...
class ShiftOverrideController {
  async setOverride(req, res) {
    try {
      const result = await shiftOverrideService.setOverride(
      ...
      return res
        .status(200)
        .json({ ...result, data: result.override || result });
    } catch (error) {
      return res
  ...
  async deleteOverride(req, res) {
    try {
      ...
      const result = await shiftOverrideService.deleteOverride(
        req.user.organizationId,
        employeeId,
        date,
      );
  ...
  async getEmployeeOverrides(req, res) {
    try {
      ...
      if (
        ...
        req.user.employee?.id !== employeeId
      ) {
        return res
          .status(403)
          .json({
            success: false,
            message: "Unauthorized to view overrides of another employee",
          });
      }
      const result = await shiftOverrideService.getEmployeeOverrides(
        req.user.organizationId,
        employeeId,
        req.query,
      );
      return res
        .status(200)
        .json({ success: true, overrides: result, data: result });
    } catch (error) {
      return res
        .status(error.statusCode || 500)
        .json({ success: false, message: error.message });
  ...
  async getOrganizationOverrides(req, res) {
    try {
      const result = await shiftOverrideService.getOrganizationOverrides(
        req.user.organizationId,
        req.query,
      );
      return res
