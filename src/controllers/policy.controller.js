...
class PolicyController {
  ...
  async upsertPolicy(req, res) {
    try {
      const result = await policyService.upsertPolicy(req.user.organizationId, req.body);
      return res.status(200).json({ success: true, message: "Attendance policy updated", policy: result });
    } catch (error) {
      return res.status(error.statusCode || 500).json({ success: false, message: error.message });
    }
  }
